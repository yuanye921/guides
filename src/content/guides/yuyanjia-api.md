...
现在 API 站准备了三条入口：主地址、香港备用、直连备用。大部分人直接用主地址就好；如果个别地区运营商抽风，再切到香港备用或直连备用试试。

注意：直连备用是 `http://42.240.142.57:3000/v1`，不是 `https://`。小手机或其他必须 `https://` 开头的应用不能用它，遇到这种情况就用主地址或香港备用。
...
**为了给大家最好的体验，我把入口拆成了主地址、香港备用和直连备用三种走法。**
